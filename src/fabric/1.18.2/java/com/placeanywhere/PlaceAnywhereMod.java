    ...
    public void onInitialize() {
        ...
        ServerPlayNetworking.registerGlobalReceiver(FreeBlockInteractPayload.ID,
                (server, player, handler, buf, responseSender) -> {
                    FreeBlockInteractPayload payload = FreeBlockInteractPayload.decode(buf);
                    server.execute(() -> FreeBlockInteractHandler.handle(payload, player));
                });

        // 命令注册：作为放置自由方块的测试入口
        CommandRegistrationCallback.EVENT.register((dispatcher, dedicated) -> {
                FreeBlocks.registerCommand(dispatcher);
